...
/** Walk object tree to find arrays that look like conversation lists (IMPROVEMENT: deep discovery). */
function findConversationArrays(root: unknown, depth = 0): unknown[] {
  if (depth > 6) return [];
  if (Array.isArray(root)) {
    if (!root.length) return [];
    // Heuristic: array of objects with message-like fields
    const sample = root[0];
    if (sample && typeof sample === "object") {
      ...
      if (
        ...
        // If items look like messages not conversations, wrap as one chat
        if ((o.role || o.sender) && (o.content || o.text) && !o.messages && !o.responses) {
          return [{ id: "imported-thread", title: "Imported thread", messages: root }];
        }
        return root;
      }
    }
  ...
  for (const key of preferredKeys) {
    ...
    if (nested) {
      for (const k2 of preferredKeys) {
        if (Array.isArray(nested[k2]) && (nested[k2] as unknown[]).length) {
          return nested[k2] as unknown[];
        }
      }
    }
  }

  if (obj.messages || obj.responses || obj.title || obj.conversation_id || obj.id) {
    if (Array.isArray(obj.messages) || Array.isArray(obj.responses)) return [obj];
  }
  ...
  for (const v of Object.values(obj)) {
    if (Array.isArray(v) && v.length && v[0] && typeof v[0] === "object") {
      ...
      if (o.messages || o.responses || o.title || o.conversation_id) return v;
    }
    if (asObject(v)) {
      const found = findConversationArrays(v, depth + 1);
...
function parseJsonText(text: string, options: ParseOptions = {}, sourceFormat = "json"): ParseResult {
  ...
  try {
    ...
    if (!items.length) throw new Error("Could not parse file as JSON or NDJSON.");
    data = items;
    sourceFormat = "ndjson";
  }
  ...
  rawList.forEach((raw, i) => {
    ...
    if (!conv) return;
    if (conv.messages.length > 0 || (conv.title && conv.title !== "Untitled conversation")) {
      conversations.push(conv);
    }
    if (rawList.length) {
      options.onProgress?.(0.3 + (0.6 * (i + 1)) / rawList.length, `Conversation ${i + 1} of ${rawList.length}`);
    }
  });
...
async function readAsArrayBuffer(file: Blob | ArrayBuffer | Uint8Array): Promise<ArrayBuffer> {
...
export async function parseGrokExport(
  ...
  if (typeof file === "string") return parseJsonText(file, options, "json-string");

  const name =
    typeof File !== "undefined" && file instanceof File ? file.name.toLowerCase() : "";
  ...
  if (name.endsWith(".zip") || looksLikeZip(buf)) {
    ...
    const jsonFiles = Object.keys(zip.files).filter(
      (p) =>
        !zip.files[p].dir &&
        (p.toLowerCase().endsWith(".json") || p.toLowerCase().endsWith(".ndjson")) &&
        !p.includes("__MACOSX"),
    );
    if (!jsonFiles.length) {
      throw new Error("ZIP opened, but no JSON files were found inside.");
    }
    jsonFiles.sort((a, b) => {
    ...
    for (let i = 0; i < jsonFiles.length; i++) {
      ...
      options.onProgress?.(0.2 + (0.5 * i) / jsonFiles.length, `Reading ${path.split("/").pop()}…`);
      const text = await zip.files[path].async("string");
      try {
        const partial = parseJsonText(text, { ...options, onProgress: undefined }, `zip:${path}`);
        ...
        warnings.push(`Skipped ${path}: ${e instanceof Error ? e.message : String(e)}`);
      }
    }
